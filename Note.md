...
以上是对八叉树一次遍历的描述，自顶向下细分八叉树需要三个步骤：①标记获取需要划分的子节点；②每次划分子节点时，分配8个子节点区域；③初始化新节点；

自定向下细分八叉树需要用到OpenGL4.3更新的计算着色器，早期没有计算着色器时，选择使用顶点着色器来处理体素列表，大概流程是向顶点着色器发送体素列表，使用顶点着色器内建成员gl_VertexID来标识每一个线程，每一个线程对应一个体素列表数据，体素列表可能有空的部分，这些内容不需要被处理，gl_VertexID由于是从0开始的索引，所以也可用作缓冲区的索引。另外为了启动正确数量的线程，需要使用glDrawArraysIndirect来直接读取GPU缓冲区中原子计数器中的内容，设置顶点数量，glDrawArraysIndirect可以避免数据从GPU到CPU造成的性能损耗。

计算着色器是独立于渲染管线的部分，有了它就不需要绕过渲染管线的固定部分来处理缓冲区数据。
